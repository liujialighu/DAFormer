...
def parse_args(args):
    ...
    parser.add_argument(
        ...
        default='npy',
        choices=['npy', 'png', 'tif'],
        help='target domain data format: npy / png / tif')
    parser.add_argument(
        '--target-img-dir',
        type=str,
        default=None,
        help='target domain image directory (for png/tif format)')
    parser.add_argument(
        '--output-dir',
        type=str,
        default=None,
        help='output directory to save logs and models, '
             'same as --work-dir, takes priority over config')
    args = parser.parse_args(args)
    if 'LOCAL_RANK' not in os.environ:
        os.environ['LOCAL_RANK'] = str(args.local_rank)
...
def main(args):
    ...
    # 根据 --target-format 动态修改目标域配置
    if args.target_format == 'png':
        assert args.target_img_dir is not None, \
            'png 格式需要指定 --target-img-dir'
        cfg.data.train.target = dict(
            type='NpyDataset',
            data_type='png',
            img_dir=args.target_img_dir,
            ann_dir=None,
        )
    elif args.target_format == 'tif':
        assert args.target_img_dir is not None, \
            'tif 格式需要指定 --target-img-dir'
        cfg.data.train.target = dict(
            type='NpyDataset',
            data_type='tif',
            img_dir=args.target_img_dir,
        )
    # npy 格式直接用 config 里的默认配置，不需要修改

    # set cudnn_benchmark
    if cfg.get('cudnn_benchmark', False):
        torch.backends.cudnn.benchmark = True

    # 输出路径优先级：--output-dir > --work-dir > config > 默认
